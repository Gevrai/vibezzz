...
export const GET: RequestHandler = async ({ params, request }) => {
	...
	const requestedRunId = parseInt(params.runId, 10);
	if (isNaN(requestedRunId)) throw error(400, 'Invalid run ID');

	const activeRun = getActiveRun(projectPath);

	// Only stream live logs if the requested runId matches the active run
	if (activeRun && activeRun.entry.id === requestedRunId) {
	...
	// Serve historical log from file
	...
	const entry = entries.find((e) => e.id === requestedRunId);
	if (entry?.log_path) {
